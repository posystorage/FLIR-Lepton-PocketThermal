...
void SDH_Set_clock(SDH_T *sdh, uint32_t sd_clock_khz)
{
    ...
    /* initial state, clock source use HIRC 这里一开始就是从PLL分的 切到HIRC再回去反而会bug 还不如一直就用PLL*/
...
//    }
    ...
    {
        u32SD_PwrCtl = CLK->PWRCTL;
				if (u32SD_PwrCtl & CLK_PWRCTL_HXTEN_Msk)//HXT ok 则直接从HXT分频
				{
					if (sdh == SDH0)
					{
							u32SD_ClkSrc = (CLK->CLKSEL0 & CLK_CLKSEL0_SDH0SEL_Msk);
							CLK->CLKSEL0 = (CLK->CLKSEL0 & ~CLK_CLKSEL0_SDH0SEL_Msk) | CLK_CLKSEL0_SDH0SEL_HXT;
					}
					else
					{
							u32SD_ClkSrc = (CLK->CLKSEL0 & CLK_CLKSEL0_SDH1SEL_Msk);
							CLK->CLKSEL0 = (CLK->CLKSEL0 & ~CLK_CLKSEL0_SDH1SEL_Msk) | CLK_CLKSEL0_SDH1SEL_HXT;
					}
					_SDH_ReferenceClock = (__HXT / 1000ul);				
				}
				else
				{
					if ((u32SD_PwrCtl & CLK_PWRCTL_HIRCEN_Msk) != 0x4ul)
